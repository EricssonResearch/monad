...
class DB():
    ...
    def getRequestsFromDB(self, start, end):
        ...
        '''
        reqs = []
        requests = self.db.TravelRequest.find({"$and": [{"startTime": {"$gte": start}}, {"startTime": {"$lt": end}}]}, {"startTime": 1, "startBusStop": 1, "endBusStop": 1, "_id": 0})  # New collection for LookAhead
        for req in requests:
