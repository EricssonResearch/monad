...
global skills: [SQLSKILL] {
	...
	//string col_userId <- string(copy_between(regular_user2 column_at 0, 1, regular_user2.rows - 1));
    //string col_stTime  <- string(copy_between(regular_user2 column_at 1,3,regular_user2.rows - 1));
    //string col_edTime  <- string(copy_between(regular_user2 column_at 2,3,regular_user2.rows - 1));
    //string col_stPosition  <- string(copy_between(regular_user2 column_at 3,4,regular_user2.rows - 1));
    //string col_edPosition  <- string(copy_between(regular_user2 column_at 4,5,regular_user2.rows - 1));
...
/*	
		...
		*/
			...
			if user_name != regular_user[0,i] {
				
				if (regular_user2[1,i] = "null"){
					up_to_date_stTime <- "null";
					string request_time_Date <- copy_between(request_time,0,10);
					string reg_end_time <-copy_between(regular_user2[2,i],10,16);
					up_to_date_edTime <- request_time_Date + reg_end_time;
					//write up_to_date_edTime;
				}else {
					up_to_date_edTime <- "null";
					string request_time_Date <- copy_between(request_time,0,10);
					string reg_start_time <-copy_between(regular_user2[1,i],10,16);
					up_to_date_stTime <- request_time_Date + reg_start_time;
					//write up_to_date_stTime;
				}
	
				//write regular_user2[1,1];
				//write reg_start_time;
				
				
				
				
				save ["userId=" + user_name + "&" + up_to_date_stTime + "&" + up_to_date_edTime + "&" + request_time  + "&stPosition=" + regular_user[3,i] + "&edPosition=" + regular_user[4,i] + "&priority=" + priority] 
