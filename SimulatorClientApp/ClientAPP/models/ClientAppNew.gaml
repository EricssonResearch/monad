...
global skills: [SQLSKILL] {
	...
	file bus_img <- image_file("../includes/bus.jpg");
	
	//list<point> distribution <- [];
	int counter_req <- 0;
	map busstop_req_map <- ();
	
	//Four counters
	int weekday_nor_req_count <- 0;
	int weekday_reg_req_count <- 0;
	int weekend_nor_req_count <- 0;
	int weekend_reg_req_count <- 0;
...
species client skills: [SQLSKILL] {
	...
	float va_titude <- (rnd(999))/1000000 update: (rnd(999))/1000000;
	...
	*/
		...
		if regular_user_flag = 0 {
			...
			if (hot_st_lgt != 0 and hot_ed_lgt !=0){
				...
				if passenger_cal_flag = 1 {
               	...
               	save [""+user_name + start_time +  end_time  + start_position +  end_position]
		    			to: "Regular_user_info" type:csv;
	...
	aspect base {
		...
		loop index_cor from: 0 to: length (list_cordinate) - 1 {
			draw  file(station_img) /*circle(6) border:#green*/ size:20 at:list_cordinate[index_cor];
				loop i from: 0 to: length(bus_stop_list) - 1{
					draw  string(spname_ls2[index_cor]) + ";" + string(bus_stop_list[index_cor][1]) + ":" +string(bus_stop_list[index_cor][1]) color: #black size:7 at:list_string_cordinate[index_cor];
				}
		}
...
experiment ClientApp type: gui {
	...
	output {
		...
		display chart_display refresh_every: 1 { 
         	chart "Requsts" type: series {
            data "Number of weekday normal requests " value: weekday_nor_req_count style: line color: rgb("magenta") ;
            data "Number of weekday regular requests " value: weekday_reg_req_count style: line color: rgb("red") ;
            data "Number of weekend normal requests " value: weekend_nor_req_count style: line color: rgb("green") ;
            data "Number of weekend regular requests " value: weekend_reg_req_count style: line color: rgb("blue") ;
         }
      	}
